...
public class LEVEL_M_TEST : MonoBehaviour {
	...
	void FillList()
    {
        foreach(var level in LevelList)
        {
            ...
            if (PlayerPrefs.GetInt("Stage" + button.LevelText.text) == 1)
            {
                level.Unlock = 1;
            ...
            button.GetComponent<Button>().onClick.AddListener(() => LoadLevel("Stage" + button.LevelText.text));
    ...
    {
        {
            ...
            foreach (GameObject buttons in allbuttons)
            {
                ...
                PlayerPrefs.SetInt("Stage" + button.LevelText.text, button.unlocked);
            }
        }
    }
